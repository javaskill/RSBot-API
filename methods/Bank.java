...
public class Bank {
	...
	public static boolean deposit(final int itemID, final int number) {
		if (isOpen() || isDepositOpen()) {
			...
			if (!isOpen()) {
				boolean match = false;
				for (int i = 0; i < 28; i++) {
					final WidgetChild comp = Widgets.get(11).getChild(17).getChild(i);
				...
				item = Inventory.getItem(itemID).getWidgetChild();
	...
	public static boolean depositAllExcept(final int... items) {
		if (isOpen() || isDepositOpen()) {
			...
			outer:
			for (int i = 0; i < 28; i++) {
				final WidgetChild item = isOpen() ? Inventory.getItemAt(i).getWidgetChild() : Widgets.get(11).getChild(17).getChild(i);
	...
	public static int getCount(final int... items) {
		...
		for (final Item item : inventoryArray) {
			for (final int id : items) {
				if (item.getId() == id) {
	 ...
	 */
	public static Widget getInterface() {
	 ...
	 */
	public static Item getItem(final Object item) {
		final Item[] Items = getItems();
		if (Items != null) {
			for (final Item Item : Items) {
	...
	public static Point getItemPoint(final int slot) {
		...
		final Item item = getItemAt(slot);
		if (item != null) {
	...
	public static Item[] getItems() {
		...
		final WidgetChild[] components = getInterface().getChild(INTERFACE_BANK_INVENTORY).getChildren();
		final Item[] items = new Item[components.length];
	...
	public static boolean open() {
		...
		try {
			...
			try {
				Calculations.distanceTo((SceneObject)banks[0]);
				Calculations.distanceTo((SceneObject) banks[1]);
				Calculations.distanceTo((NPC) banks[2]);
			} catch(Exception e){}
	...
	private static boolean scrollTo(final WidgetChild component, final WidgetChild scrollBar) {
	...
	public static boolean withdraw(final int itemID, final int count) {
		...
		final Item rsi = getItem(itemID);
		if (rsi == null || rsi.getId() == -1) {
			return false;
		}
		final WidgetChild item = rsi.getWidgetChild();
		if (item == null) {
			return false;
		}
		int t = 0;
	...
	public static WidgetChild getComponent(final int id) {
		final int x = id >> 16;
		final int y = id & 0xFFFF;
		return Widgets.get(x).getChild(y);
	}
	/**
	 * Tries to withdraw an item.
	 * 0 is All. -1 is All but one, 1, 5, 10 use Withdraw 1, 5, 10 while other numbers Withdraw X.
	 ...
	 */
	public static Item[] getEquipmentItems() {
	 ...
	 * @return RSItem
	 */
	public static Item getEquipmentItem(final int id) {
		final Item[] items = getEquipmentItems();
		if (items != null) {
			for (final Item item : items) {
				if (item.getId() == id) {
					return item;
